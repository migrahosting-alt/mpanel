...
export default function DatabaseManagement() {
  ...
  const fetchDatabases = async () => {
    try {
      setLoading(true);
      const response = await apiClient.get('/database-mgmt');
      setDatabases(response.data || response);
    } catch (error) {
  ...
  const handleCreateDatabase = async () => {
    try {
      await apiClient.post('/database-mgmt', createForm);
  ...
  const handleDeleteDatabase = async (id, name) => {
    ...
    try {
      await apiClient.delete(`/database-mgmt/${id}`);
  ...
  const handleViewUsers = async (database) => {
    try {
      ...
      const response = await apiClient.get(`/database-mgmt/${database.id}/users`);
      setDbUsers(response.data || response);
  ...
  const handleAddUser = async () => {
    try {
      await apiClient.post(`/database-mgmt/${selectedDatabase.id}/users`, userForm);
  ...
  const handleDeleteUser = async (username) => {
    ...
    try {
      await apiClient.delete(`/database-mgmt/${selectedDatabase.id}/users/${username}`);
  ...
  const handleViewStats = async (database) => {
    try {
      setSelectedDatabase(database);
      const response = await apiClient.get(`/database-mgmt/${database.id}/stats`);
      setDbStats(response.data || response);
  ...
  const handleExport = async (database) => {
    try {
      // Note: apiClient may need enhancement for blob responses
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000/api'}/database-mgmt/${database.id}/export`, {
        credentials: 'include',
