...
export default function FileManager() {
  ...
  const fetchFiles = async (pathToLoad = '') => {
    try {
      setLoading(true);
      const response = await apiClient.get('/files/browse', {
        params: { path: pathToLoad || '' }
      });
      setFiles(response.files || response.entries || []);
      if (typeof response.homePath === 'string') {
        setHomePath(response.homePath.replace(/\/+$/, ''));
      }
    } catch (error) {
  ...
  const toRelativePath = (absPath = '') => {
    if (!absPath) return '';
    if (!homePath) return absPath.replace(/^\/+/, '');
    const normalizedHome = homePath.endsWith('/') ? homePath : `${homePath}/`;
    if (absPath === homePath) return '';
    if (absPath.startsWith(normalizedHome)) {
      return absPath.slice(normalizedHome.length).replace(/^\/+/, '');
    }
  ...
  const navigateToFolder = (folderPath) => {
    if (folderPath === '..') {
      ...
      setCurrentPath(normalizeRelativePath(folderPath));
    }
  };
  ...
  const handleUpload = async (event) => {
    ...
    formData.append('path', normalizeRelativePath(currentPath));
  ...
  const handleDownload = async (file) => {
    try {
      ...
      const response = await fetch(`${API_BASE}/files/download?path=${encodeURIComponent(relativePath)}`, {
        credentials: 'include',
        headers: {
  ...
  const handleEdit = async (file) => {
    try {
      const response = await apiClient.get('/files/content', {
        params: { path: toRelativePath(file.path) }
      });
      setEditingFile(file);
  ...
  const handleSaveFile = async () => {
    try {
      await apiClient.post('/files/content', {
        path: toRelativePath(editingFile.path),
        content: fileContent
      });
  ...
  const handleCreateDirectory = async () => {
    ...
    try {
      await apiClient.post('/files/mkdir', {
        path: normalizeRelativePath(currentPath),
        name: newDirName
      });
  ...
  const handleCompress = async () => {
    ...
    try {
      const paths = selectedFiles.map(f => toRelativePath(f.path));
  ...
  const handleExtract = async (file) => {
    ...
    try {
      await apiClient.post('/files/extract', {
        path: toRelativePath(file.path),
        destination: normalizeRelativePath(currentPath)
      });
  ...
  const handleAiSubmit = async () => {
    ...
    try {
      const response = await apiClient.post('/ai/files/explain', {
        path: toRelativePath(selectedAiFile.path),
        question: aiQuestion || undefined
      });
      setAiResponse(response.explanation || JSON.stringify(response, null, 2));
    } catch (error) {
      console.error('Error asking AI:', error);
      setAiResponse(error.message || 'AI service unavailable. Make sure OPENAI_API_KEY is configured.');
    } finally {
      setAiLoading(false);
    }
  };
                  ...
                  <td
                    ...
                    onDoubleClick={() => {
                      if (file.type === 'directory') {
                        navigateToFolder(toRelativePath(file.path));
