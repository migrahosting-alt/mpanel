...
export default function Provisioning() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch stats
      const statsRes = await fetch('/api/provisioning/stats', { headers });
      if (statsRes.ok) {
      ...
      // Fetch tasks
      const tasksUrl = filterStatus === 'all' 
        ? '/api/provisioning/tasks'
        : `/api/provisioning/tasks?status=${filterStatus}`;
      ...
      // Fetch failed jobs
      const failedRes = await fetch('/api/provisioning/failed', { headers });
      if (failedRes.ok) {
  ...
  const retryTask = async (taskId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`/api/provisioning/retry/${taskId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const manualProvision = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('/api/provisioning/provision', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (response.ok) {
        ...
        toast.success(`Provisioning job created: ${data.jobId}`);
        fetchData();
      } else {
        toast.error('Failed to create provisioning job');
      }
    } catch (error) {
  ...
  const clearFailedJobs = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('/api/provisioning/failed', {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-gray-500">
                          Attempt {job.attemptsMade} of {job.opts?.attempts || 3}
                        </p>
                        {job.failedReason && (
                      ...
                      <button
                        onClick={() => retryTask(job.data?.taskId)}
