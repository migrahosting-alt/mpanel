...
const getNavigation = (isAdmin, hasPermission) => {
  ...
  const hostingNav = [
    ...
    { name: 'Premium Tools', href: '/premium-tools', icon: SparklesIcon, section: 'hosting', badge: '33' },
  ];
  ...
  const allNav = isAdmin 
    ? [...baseNav, ...filterByPermission(adminNav), ...filterByPermission(hostingNav), ...filterByPermission(billingNav), ...securityNav]
...
const getSections = (isAdmin) => {
  ...
  return isAdmin ? { main: 'Overview', admin: 'Administration', ...base } : base;
};
