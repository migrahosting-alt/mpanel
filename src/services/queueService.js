...
import { twilioCircuitBreaker } from '../utils/circuitBreaker.js';
...
class QueueService {
  ...
  async addSMSJob(smsData) {
    ...
    logger.info(`SMS job added to queue: ${job.id}`, { to: smsData.to });
  ...
  startWorkers() {
    ...
    // SMS worker
    this.processQueue(this.queues.sms, async (job) => {
      return await this.handleSMSJob(job);
    });
  ...
  async handleEmailJob(job) {
    ...
    const htmlTemplates = (await import('./htmlEmailTemplates.js')).default;
    
    // If HTML template specified, generate HTML content
    if (job.data.htmlTemplate && htmlTemplates[job.data.htmlTemplate]) {
      job.data.htmlContent = htmlTemplates[job.data.htmlTemplate](job.data.templateData || {});
    }
    ...
    await emailService.sendTransactional({
      ...
      data: job.data.data,
      htmlContent: job.data.htmlContent // Pass pre-rendered HTML
    });
  ...
  async handleSMSJob(job) {
    try {
      ...
      const message = await twilioCircuitBreaker.execute(async () => {
      ...
      // Update sms_queue table if record exists
      if (job.data.queueRecordId) {
        ...
        await pool.query(
          `UPDATE sms_queue 
           SET status = 'sent', 
               ...
               updated_at = NOW()
           WHERE id = $2`,
          [message.sid, job.data.queueRecordId]
        );
      }
      ...
      // Update sms_queue table with error if record exists
      if (job.data.queueRecordId) {
        const pool = (await import('../db/index.js')).default;
        await pool.query(
          `UPDATE sms_queue 
           SET status = 'failed', 
               ...
               updated_at = NOW()
           WHERE id = $2`,
          [error.message, job.data.queueRecordId]
        );
      }
  ...
  async handleFailedJob(job, error) {
    ...
    // Send alert to admins using error alert service
    try {
      const { alertQueueFailure } = await import('./errorAlertService.js');
      await alertQueueFailure(job.type, job.data, error);
    } catch (emailError) {
