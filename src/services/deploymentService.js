...
class DeploymentService {
  ...
  async getAllDeployments(filters = {}) {
    ...
    let query = 'SELECT d.*, s.name as server_name, u.email as user_email FROM deployments d LEFT JOIN servers s ON d.server_id = s.id LEFT JOIN users u ON d.user_id = u.id WHERE 1=1';
    const params = [];
    let paramCount = 1;
  ...
  async getDeploymentById(deploymentId) {
    try {
      const result = await pool.query(
        'SELECT d.*, s.name as server_name, u.email as user_email FROM deployments d LEFT JOIN servers s ON d.server_id = s.id LEFT JOIN users u ON d.user_id = u.id WHERE d.id = $1',
        [deploymentId]
      );
