...
router.post('/bulk-action', authenticateToken, requireRole(['super_admin', 'admin', 'manager']), async (req, res) => {
  try {
    ...
    let result;
    const placeholders = customer_ids.map((_, i) => `$${i + 3}`).join(', ');
    ...
    switch (action) {
      case 'delete':
        ...
        if (role === 'super_admin') {
          result = await pool.query(
            `UPDATE customers SET deleted_at = NOW(), deleted_by = $1 WHERE id IN (${placeholders}) RETURNING id`,
            [user_id, ...customer_ids]
          );
        } else {
          result = await pool.query(
            `UPDATE customers SET deleted_at = NOW(), deleted_by = $1 WHERE tenant_id = $2 AND id IN (${placeholders}) RETURNING id`,
      ...
      case 'activate':
        if (role === 'super_admin') {
          result = await pool.query(
            `UPDATE customers SET status = 'active', updated_at = NOW() WHERE id IN (${placeholders}) RETURNING id`,
            [...customer_ids]
          );
        } else {
          result = await pool.query(
            `UPDATE customers SET status = 'active', updated_at = NOW() WHERE tenant_id = $1 AND id IN (${placeholders}) RETURNING id`,
      ...
      case 'suspend':
        if (role === 'super_admin') {
          result = await pool.query(
            `UPDATE customers SET status = 'suspended', updated_at = NOW() WHERE id IN (${placeholders}) RETURNING id`,
            [...customer_ids]
          );
        } else {
          result = await pool.query(
            `UPDATE customers SET status = 'suspended', updated_at = NOW() WHERE tenant_id = $1 AND id IN (${placeholders}) RETURNING id`,
            [tenant_id, ...customer_ids]
          );
        }
        break;
      ...
      case 'restore':
        ...
        if (role === 'super_admin') {
          result = await pool.query(
            `UPDATE customers SET deleted_at = NULL, deleted_by = NULL, updated_at = NOW() WHERE id IN (${placeholders}) RETURNING id`,
            [...customer_ids]
          );
        } else {
          result = await pool.query(
            `UPDATE customers SET deleted_at = NULL, deleted_by = NULL, updated_at = NOW() WHERE tenant_id = $1 AND id IN (${placeholders}) RETURNING id`,
            [tenant_id, ...customer_ids]
          );
        }
        break;
